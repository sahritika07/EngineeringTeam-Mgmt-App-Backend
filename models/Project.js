...
//     this.progress = 0
//   } else {
//     // Group assignments by engineer (assignee)
//     const engineerProgressMap = {}

//     assignments.forEach((a) => {
//       const engineerId = a.assignee?.toString()
//       if (!engineerId) return

//       if (!engineerProgressMap[engineerId]) {
//         engineerProgressMap[engineerId] = { total: 0, count: 0 }
//       }
//       engineerProgressMap[engineerId].total += a.progress || 0
//       engineerProgressMap[engineerId].count += 1
//     })

//     // Calculate average progress per engineer
//     const engineerAverages = Object.values(engineerProgressMap).map(
//       (engineer) => engineer.total / engineer.count
//     )

//     // Calculate final project progress as average of engineer averages
//     this.progress = Math.round(
//       engineerAverages.reduce((sum, avg) => sum + avg, 0) / engineerAverages.length
//     )
//   }
...
// // ✅ Budget remaining virtual
// ProjectSchema.virtual("budgetRemaining").get(function () {
//   return this.budget - this.budgetUsed;
// });

// // ✅ Days until deadline virtual
// ProjectSchema.virtual("daysUntilDeadline").get(function () {
//   const now = new Date();
//   const deadline = new Date(this.deadline);
//   const diffTime = deadline - now;
...
// ✅ Update progress when called (simplified)
ProjectSchema.methods.updateProgress = async function () {
  const Assignment = mongoose.model("Assignment");
  const assignments = await Assignment.find({ project: this._id });
  ...
  if (assignments.length === 0) {
    ...
    // Simple average of all assignment progress
    const total = assignments.reduce((sum, a) => sum + (a.progress || 0), 0);
    this.progress = Math.round(total / assignments.length);
  }
